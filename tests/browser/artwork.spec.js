...
async function mockTools(page) {
  const plans = new Map(), outputs = new Map(), requests = [];
  let sequence = 0;
...
test('whole JS and SVG scenes render and download', async ({ page }) => {
  ...
  for (const [type, source, expected] of [['js', jsScene, [255, 20, 147, 255]], ['svg', svgScene, [0, 250, 154, 255]]]) {
    ...
    const results = await tools.run([{ name: `draw_${type}`, args: { [key]: source } }]);
    expect(JSON.parse(results[0]).error).toBeUndefined();
    const image = type === 'js'
      ? await page.frameLocator('.drawing-native-canvas').locator('canvas').evaluate(canvas => canvas.toDataURL())
      : await page.locator('#artwork-layer').getAttribute('href');
    expect(await pixels(page, image)).toEqual([expected]);
    const download = page.waitForEvent('download');
    await page.locator('#draw-export').click();
    expect((await download).suggestedFilename()).toBe(`agent-drawing.${type === 'js' ? 'png' : 'svg'}`);
  }
  await expect(page.locator('.drawing-native-canvas')).toHaveCount(0);
});

test('JavaScript failures retain the previous picture; reset and navigation cancel workers', async ({ page }) => {
  const tools = await mockTools(page);
  const initial = { name: 'draw_svg', args: { svg: svgScene } };
  ...
  for (const code of ['const = broken', 'throw new Error("runtime-probe")', 'while(true){}']) {
    const before = tools.requests.length;
    await tools.run([{ name: 'draw_js', args: { code } }]);
    await expect(page.locator('#draw-error')).toContainText('JavaScript error');
    expect(tools.requests.length - before).toBe(1);
  ...
  for (const cancel of ['reset', 'navigate']) {
    ...
    await expect(page.locator('.drawing-native-canvas')).toBeAttached();
...
test('sandbox and SVG image context cannot read page state or request external resources', async ({ page }) => {
  ...
  const result = await tools.run([{ name: 'draw_svg', args: { svg } }]);
  expect(JSON.parse(result[0]).error).toBeUndefined();
  expect(await pixels(page, await page.locator('#artwork-layer').getAttribute('href'))).toEqual([[102, 51, 153, 255]]);
...
test('replacement clears old shapes; Chat and the real Voice executor share artwork', async ({ page }) => {
  const tools = await mockTools(page);
  await tools.run([{ name: 'draw_svg', args: { svg: svgScene } }]);
  await tools.run([{ name: 'draw_js', args: { code: jsScene } }]);
  await expect(page.locator('#shape-layer *')).toHaveCount(0);
  await expect(page.locator('#artwork-layer')).toHaveCount(0);
  await expect(page.frameLocator('.drawing-native-canvas').locator('canvas')).toBeVisible();
  ...
  await tools.run([{ name: 'update_svg', args: { find: 'fill="#00fa9a"', replace: 'fill="#ef5350"' } }]);
  expect(await pixels(page, await page.locator('#artwork-layer').getAttribute('href'))).toEqual([[239, 83, 80, 255]]);
});
