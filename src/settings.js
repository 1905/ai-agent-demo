...
import { mountMicrophoneSettings } from './microphone-settings.js';
...
export function mountSettings(root, { language = 'en', onChange = () => {}, voiceConnected = () => false } = {}) {
  ...
  const microphoneSettings = mountMicrophoneSettings(dialog, { voiceConnected });
  root.querySelector('.settings-button').onclick = () => { dialog.showModal(); microphoneSettings.open(); };
  dialog.onclick = event => {
    if (event.target !== dialog) return;
    const rect = dialog.getBoundingClientRect();
    if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) dialog.close();
  };
  return () => microphoneSettings.dispose();
}
